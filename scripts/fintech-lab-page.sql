-- Cedar-IBSi FinTech Lab: create the page table and load its content.
-- Run by hand in pgAdmin against the Azure database: paste the whole file into
-- the Query Tool and execute it as one buffer.
--
...
--
-- The last statement records the migration in schema_migrations, so a later
-- `npm run migrate` skips 021 instead of applying it a second time. That keeps
-- the file in migrations/ (a fresh database still needs the CREATE TABLE) and
-- the ledger honest about what has already run here.
...
VALUES ('021_fintech_lab_page.sql')
ON CONFLICT (filename) DO NOTHING;

-- Read this back before committing: one row, five stats, four benefit groups.
SELECT
  title,
  length(story)                AS story_chars,
  jsonb_array_length(stats)    AS stat_count,
  jsonb_array_length(benefits) AS benefit_count
FROM fintech_lab_page
WHERE id = 1;
